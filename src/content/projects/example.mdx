...
_For more info on using remote fonts (Google Fonts, etc.), check the [Astro Font Provider Reference.](https://docs.astro.build/en/reference/font-provider-reference/)_
...
_Note: I'm not a pro with this specific setup yet, so take this with a grain of salt._
...
const turnstileField = import.meta.env.DEV
  ? z.string().optional().default("") // dev: missing → ''
  ...
  newsletterSubscribe: defineAction({
    ...
    handler: async ({ email, "cf-turnstile-response": token }, ctx) => {
      ...
      try {
        ...
        if (!upsertRes.ok) {
          console.error("Kit upsert failed", {
        ...
        if (!attachRes.ok) {
          console.error("Kit form attach failed", {
